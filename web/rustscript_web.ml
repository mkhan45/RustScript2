...
let web_state () =
    let static_atoms = Rustscript.Run.base_static_atoms () in
    let static_idents = Rustscript.Run.base_static_idents () in
    let static_block_funcs = [] in
    let call_stack = [] in
    let fold_step (ss, state) in_string =
        Rustscript.Run.run_string 
            in_string
            ("stdlib")
            (ss, state)
    in
    let init_state: (Rustscript.Types.static_state * Rustscript.Types.state) =
        ({static_atoms; static_idents; static_block_funcs; call_stack}, (Map.empty (module Int)))
    in
    List.fold_left ~init:init_state ~f:fold_step (stdlib ())

let _ =
    let (ss, state) = web_state () in
    Js.export "rustscript"
        (object%js (_self)
           method run in_string filename = 
